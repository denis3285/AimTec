    ...
    public static class EvadeManager
    {
        ...
        public static void Attach(Menu mainMenu)
        {
            ...
            EvadeSpellMenu = new Menu("evadeSpells", "Spell Settings");
            {
                ...
                {
                    var name = spell.Slot;
                    EvadeSpellMenu.Add(new MenuSeperator("FioraEvade_" + name,
                        ObjectManager.GetLocalPlayer().ChampionName + " " + name + " Settings"));
            ...
            SkillShotMenu = new Menu("Skillshots", "Skillshot Settings");
            {
                        ...
                        i => GameObjects.EnemyHeroes.Any(a => a.ChampionName == i.ChampionName)))
                {
                    ...
                    heroMenu.Add(new MenuSeperator("EvadeSpell" + spell.MenuItemName,
                        spell.SpellName + " " + spell.Slot));
                    heroMenu.Add(new MenuSlider("DangerLevel" + spell.MenuItemName, "Danger Level", spell.DangerValue,
                        1, 5));
        ...
        private static void OnUpdate()
        {
            ...
            foreach (var ally in bestAllies)
            {
                ...
                if (ObjectManager.GetLocalPlayer().IsDead || ally
                ...
                if (
                    !IsSafe(ally.ServerPosition.To2D()).IsSafe)
                {
                    
                    TryToEvade(
                        IsSafe(ally.ServerPosition.To2D())
                            ...
                            if (spellList.Any())
                            {
                                ...
                                if (willHitList.Any())
                                {
                                        ...
                                        Support_AIO.Bases.Champion.RootMenu["wset"]["health"].As<MenuSlider>().Value)
                                    {
                                        ...
                                        if (Support_AIO.BraumWE.EvadeTargetManager.Menu["whitelist"][
                                            ...
                                            .As<MenuBool>().Enabled)
                                        {
                                            if (willHitList.OrderByDescending(
                                                ...
                                                .Any(
                                                    x =>
                                                        Support_AIO.Bases.Champion.W.CastOnUnit(ally)))
                                            {
                                                return;
                                            }
                                                    ...
                                                    if (willHitList.OrderByDescending(
                                                        ...
                                                        .Any(
                                                            x =>
                                                                ...
                                                                Support_AIO.Bases.Champion.E.Cast(x.Start.To3D())))
                                                    {
        ...
        private static void OnDetectSkillshot(Skillshot skillshot)
        {
            ...
            foreach (var ally in bestAllies)
            {
                ...
                if (skillshot.Start.Distance(ally.ServerPosition.To2D()) >
        ...
        public static List<Vector2> GetEvadePoints(int speed = -1, int delay = 0, bool onlyGood = false)
        {
            ...
            foreach (var ally in bestAllies)
            {
                ...
                foreach (var skillshot in detectedSkillshots)
                {
                    if (skillshot.Evade())
                    {
                        if (skillshot.SpellData.TakeClosestPath && skillshot.IsDanger(ally.ServerPosition.To2D()))
                        {
                ...
                var myPosition = ally.ServerPosition.To2D();
                    ...
                    for (var i = 0; i <= poly.Points.Count - 1; i++)
                    {
                        ...
                        if (distanceToEvadePoint < 600 * 600)
                        {
                            ...
                            for (var j = -s; j <= s; j++)
                            {
                                ...
                                var pathToPoint = ally.GetPath(candidate.To3D())
                                    .To2DList();
