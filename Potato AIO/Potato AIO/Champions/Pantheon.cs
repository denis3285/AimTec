    ...
    class Pantheon : Champion
    {
        protected override void Combo()
        {
            ...
            if (Hmmmmm < Game.TickCount)
            {
                if (!Player.HasBuff("pantheonesound"))
                {
                    ...
                    if (E.Ready && useE && target.IsValidTarget(E.Range) && !W.Ready && !Q.Ready && !Orbwalker.Implementation.IsWindingUp)
                    {
                        if (target != null)
                        {
                            E.Cast(target);
        ...
        protected override void SetMenu()
        {
            ...
            var LaneClear = new Menu("lane", "Lane Clear");
            {
                ...
                LaneClear.Add(new MenuBool("lastq", "Use Q for Last Hit"));
        ...
        {
            Q = new Aimtec.SDK.Spell(SpellSlot.Q, 600);
            W = new Aimtec.SDK.Spell(SpellSlot.W, 600);
            E = new Aimtec.SDK.Spell(SpellSlot.E, 600);
            R = new Aimtec.SDK.Spell(SpellSlot.R, 5500);
            E.SetSkillshot(0.25f, 15f * 2 * (float)Math.PI / 180, 2000f, false, SkillshotType.Cone);
